...
export default function DetalheAviso() {
  ...
  if (!aviso) return <div className="p-6 text-center text-slate-500">Carregando…</div>

  return (
    <div className="min-h-full max-w-2xl mx-auto p-4">
      <Link to="/feed" className="text-sm text-slate-500 hover:text-slate-700 mb-4 inline-block">← Voltar</Link>
      ...
      <Card className="p-5 mb-4">
        ...
        <div className="text-xs text-slate-500 mb-4">{new Date(aviso.publicadoEm).toLocaleString('pt-BR')}</div>
      ...
      <Card className="p-5">
        ...
        <div className="space-y-3 mb-4 max-h-80 overflow-y-auto">
          {mensagens.length === 0 && <div className="text-sm text-slate-500">Sem mensagens ainda. Use o campo abaixo para falar com o síndico.</div>}
          {mensagens.map((m) => (
