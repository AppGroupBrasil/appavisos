  ...
  return (
    <div className="min-h-full">
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {ident?.descricaoCurta && <div className="text-xs text-slate-500 truncate">{ident.descricaoCurta}</div>}
          </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-4 space-y-3">
        ...
        {itens.length === 0 && <div className="text-center py-12 text-slate-500">Nenhum aviso no momento.</div>}
        {itens.map((a) => (
          <Link key={a.avisoId} to={`/aviso/${a.avisoId}`} className="block">
            <Card className="p-5 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
              ...
              <p className="text-sm whitespace-pre-wrap text-slate-700 dark:text-slate-300 line-clamp-3">{a.texto}</p>
              ...
              <div className="mt-4 text-xs">
                {a.cienteEm
                  ...
                  : <span className="text-slate-500">Toque para abrir e responder →</span>}
              </div>
            </Card>
          </Link>
        ))}
