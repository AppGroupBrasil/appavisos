          ...
          <Card key={r.id} className="p-4">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-slate-700">{r.bloco && `${r.bloco} • `}{r.apartamento && `Apto ${r.apartamento} • `}{r.email}</div>
                {r.resposta && <div className="text-sm mt-2 p-2 bg-slate-50 dark:bg-slate-800 rounded">"{r.resposta}"</div>}
                <div className="grid grid-cols-3 gap-2 mt-3 text-xs">
                  <div>
                    <div className="text-slate-600">E-mail enviado</div>
                    <div>{r.emailEnviadoEm ? new Date(r.emailEnviadoEm).toLocaleString('pt-BR') : '—'}</div>
                  </div>
                  <div>
                    <div className="text-slate-600">E-mail aberto<span title="Pode ter falsos positivos: Apple Mail e Gmail pré-carregam imagens em proxies." className="ml-1 cursor-help">ⓘ</span></div>
                    <div>{r.emailAbertoEm ? new Date(r.emailAbertoEm).toLocaleString('pt-BR') : '—'}</div>
                  </div>
                  <div>
                    <div className="text-slate-600">Visualizou no app</div>
              ...
              <div className="text-right text-sm whitespace-nowrap">
                {r.cienteEm
                  ? <span className="text-emerald-600">✓ Ciente<br/><span className="text-xs text-slate-700">{new Date(r.cienteEm).toLocaleString('pt-BR')}</span></span>
