...
export default function QrCodes() {
  ...
  return (
    <ShellSindico>
      ...
      <p className="text-sm text-slate-700 mb-6">Imprima e cole no mural ou nas áreas do condomínio.</p>

      <div className="grid md:grid-cols-2 gap-4 max-w-3xl mb-6">
        <Card className="p-5">
          <h2 className="font-semibold mb-3">QR de cadastro</h2>
          <p className="text-sm text-slate-700 mb-4">Morador escaneia, preenche cadastro e fica pendente até você aprovar.</p>
          <QrComBotao endpoint="/api/qr/cadastro.png" arquivo={`qr-cadastro-${slug}.png`} />
        </Card>
        <Card className="p-5">
          <h2 className="font-semibold mb-3">QR do mural (geral)</h2>
          <p className="text-sm text-slate-700 mb-4">Morador escaneia e vê todos os avisos atualizados — sempre o mesmo QR.</p>
          <QrComBotao endpoint="/api/qr/feed.png" arquivo={`qr-mural-${slug}.png`} />
        </Card>
        <Card className="p-5">
          <h2 className="font-semibold mb-3">QR de reportes (geral)</h2>
          <p className="text-sm text-slate-700 mb-4">Morador escaneia e abre formulário para reportar ocorrências, manutenção, reclamações, sugestões.</p>
      ...
      <p className="text-sm text-slate-700 mb-4">Cole o QR no local da área (salão, academia, piscina). Quem escaneia vê só os avisos daquela área específica.</p>
      {areas.length === 0 ? (
        <Card className="p-6 text-center text-slate-700 text-sm">Nenhuma área cadastrada. Acesse o menu <b>Áreas</b> para criar.</Card>
      ) : (
          ...
          {areas.map((a) => (
            <Card key={a.id} className="p-5">
              ...
              <div className="space-y-4">
                <div>
                  <div className="text-xs font-medium text-slate-700 mb-1">Avisos da área</div>
                  <QrComBotao endpoint={`/api/qr/area/${a.id}.png`} arquivo={`qr-area-${a.slug}.png`} />
                </div>
                <div className="pt-3 border-t border-slate-200">
                  <div className="text-xs font-medium text-slate-700 mb-1">Reportar nesta área</div>
