...
type Cond = { nome: string; logoUrl?: string; corPrimaria?: string; descricaoCurta?: string }
...
export default function MuralPublico() {
  ...
  useEffect(() => {
    api.get(`/api/publico/${slug}/mural`)
      .then(r => {
      ...
      .catch(() => setErro('Condomínio não encontrado ou indisponível.'))
  }, [slug])
  ...
  if (!cond) return <div className="p-6 text-center text-slate-500">Carregando...</div>
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {cond.descricaoCurta && (
              <div className="text-xs text-slate-500 truncate">{cond.descricaoCurta}</div>
            )}
          </div>
        </div>
      </header>
      <main className="max-w-2xl mx-auto px-4 py-4 space-y-3">
        <div className="text-xs text-slate-500 text-center py-2">Mural de avisos</div>
        {avisos.length === 0 ? (
          <div className="text-center py-16 text-slate-500">Nenhum aviso publicado no momento.</div>
        ) : (
          avisos.map(a => (
